...
- Core 为所有顶层 Main 模型保留 compact、state-aware、capability-gated、top-level、one-shot bootstrap。它按当前 workflow Skill、其他 Skills、native `task`、delegation 许可和 exact native supplied-index provenance 选择最小入口；workflow index 可见时，第一响应只在 DIRECT、`SUPPLIED BY EXACT NATIVE skill-prompt` 后直接 PLAN、或 `NOT SUPPLIED` 后 index-only read/wait 之间选择。它可以要求模型遵守上述七阶段顺序、使用结构化 `NOW/THEN`、更新自己的 TODO，并复述 OMP 已激活的原生要求，但不得独立选择 workflow、Skill、Agent、fork，增加权限，或因遗漏形成 plugin-owned completion gate、自动 retry/continuation。两个 reminder 分别由 `OMP_ENHANCER_DISABLE_WORKFLOW_REMINDER=1`（reminder off）与 `OMP_ENHANCER_DISABLE_PROTOCOL_COACH=1`（coach off）在受控诊断中关闭。
- 同一 top-level Main 任务范围内，Core 的 JSON-safe protocol coach 只观察 Main 已声明的阶段：index 后 `PRE_PLAN`，全部 NOW、visible resource extensions 与 THEN settled 后 `PRE_READY`，最终 READY 和成功 TODO 后 `PRE_DISPATCH`；EXECUTE 期间另有有界 `EXECUTE_STABILITY` 重提醒（8 个 settled tool result 一次、每 task 上限 20、不覆盖 pending phase cue、被后续 phase cue 覆盖）。`context` 只把一个隐藏临时 cue 附加到 copied messages，`message_end` 与 `tool_result` 只观察；`PRE_DISPATCH` 复述 committed Delegate row 的 exact assignment-prefix key order、非空 batch context、可直接使用而非 status/reference-only 的 terminal evidence，以及 no-op conditional repair checkpoint 的 resolved/completed 状态。它不触发下一次自然请求、不改写既有 message/tool input/result，也不选择 workflow、Skill、Agent、fork 或完成。状态在 provider retry 与 continuation 中可恢复、在真正新任务时 reset；failed declared read 只算 returned/unavailable，不伪造成 loaded。`writing.pending` 初始 READY/TODO 不产生 `PRE_DISPATCH`，只有合法 replacement language generation 才进入 dispatch。`OMP_ENHANCER_DISABLE_PROTOCOL_COACH=1` 独立关闭该 coach；不得把它实现为 block、router、gate、retry 或 completion controller。
- 打包的 Config template 不绑定具体 Main/Advisor 模型；模型选择由用户配置，reminder 与 coaching 对所有顶层 Main 模型通用。
...
Reminder 与 protocol-coach 行为由确定性 hook tests 和可选行为矩阵验证。Runner 的 `--model` 是单次全矩阵 override，接受任意用户配置的模型；下面是同一矩阵、相同 thinking/repeat 的一次独立运行示例，不是一次调用完成的 paired A/B wrapper，也不自动生成模型差分结论：
...
同样的 `--model` override 可用于 `e2e:main:subagents` 与 `e2e:main:advisor`。比较时必须保持 matrix、scenario、thinking、repeat、插件态和 evaluator 一致，并分别保留输出目录。模型 A/B、reminder-on/off A/B 与 protocol-coach-on/off A/B 是不同实验。Reminder 对照在同一模型下使用 `OMP_ENHANCER_DISABLE_WORKFLOW_REMINDER=1`（reminder off）；coach 对照只切换 `OMP_ENHANCER_DISABLE_PROTOCOL_COACH=1`（coach off）。一次 live run 只是样本，不能证明稳定提升。
