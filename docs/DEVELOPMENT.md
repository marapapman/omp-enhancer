...
├── commandcode/             # Command Code Provider API provider、/login 和 /model 接入
└── aliyun-bailian-token-plan/ # 百炼 Token Plan provider、/login 和 /model 接入
...
重要文件：
- `package.json`：六个 npm workspace 和统一脚本；
...
- `plugins/commandcode/index.js`：Command Code Provider API provider 和原生 `/login` 接入；
- `plugins/volcengine-coding-plan/index.js`：Coding Plan provider 和原生 `/login` 接入；
- `plugins/aliyun-bailian-token-plan/index.js`：Alibaba Bailian Token Plan provider 和原生 `/login` 接入；
...
- workflow catalog v40 只有 `writing`、`research` 和 `visual` 三个 advisory 域；目录不是 router、gate 或 completion controller；
