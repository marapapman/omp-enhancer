...
- 同一 top-level Main 任务范围内，Core 的 JSON-safe protocol coach 只观察 Main 已声明的阶段：index 后 `PRE_PLAN`，全部 NOW、visible resource extensions 与 THEN settled 后 `PRE_READY`，最终 READY 和成功 TODO 后 `PRE_DISPATCH`。`context` 只把一个隐藏临时 cue 附加到 copied messages，`message_end` 与 `tool_result` 只观察；`PRE_DISPATCH` 复述 committed Delegate row 的 exact assignment-prefix key order、非空 batch context、可直接使用而非 status/reference-only 的 terminal evidence，以及 no-op conditional repair checkpoint 的 resolved/completed 状态。它不触发下一次自然请求、不改写既有 message/tool input/result，也不选择 workflow、Skill、Agent、fork 或完成。状态在 provider retry 与 continuation 中可恢复、在真正新任务时 reset；failed declared read 只算 returned/unavailable，不伪造成 loaded。`writing.pending` 初始 READY/TODO 不产生 `PRE_DISPATCH`，只有合法 replacement language generation 才进入 dispatch。`OMP_ENHANCER_DISABLE_PROTOCOL_COACH=1` 独立关闭该 coach；不得把它实现为 block、router、gate、retry 或 completion controller。
