...
- Draw.io pipeline remains unchanged: task draws once with drawio-skill (drawio@365-skills) and exports the complete evidence set—preview PNG、cropped vector PDF 和修复后的 final PNG；Main（或未绘制该 revision 的 task）performs one independent read-only review of the actual exports；task applies at most one local fix round, re-exports the same set, and the same reviewer confirms the recorded findings against fresh evidence。
