...
工作流 definition 位于 `scripts/workflow-definitions.js`，校验和渲染模块分别是 `scripts/workflow-schema.js` 与 `scripts/workflow-render.js`。当前 catalog version 35 只有三个域：`writing`、`research`（事实核查）和 `visual`。
