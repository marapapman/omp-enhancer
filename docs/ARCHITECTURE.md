...
当前 visual-delivery 不变量要求：已暴露的 `designer` 拥有设计或修订 checkpoint，随后 Main 对 exact current revision 进行绑定和渲染，再由独立只读的 `visioner` 检查该证据。Agent 缺失时保留具体 checkpoint 或 evidence limitation；这只是 advisory guidance，不是 hard gate、router、fixed fanout、automatic loop 或 completion authority。
