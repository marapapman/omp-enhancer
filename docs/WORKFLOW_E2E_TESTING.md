...
> **部分已被取代。** 本文件保留旧代码/测试增强矩阵的历史证据；当前验证 `omp-config`、`writing-helper`、`omp-fact-checker`，并单独验证 `volcengine-coding-plan`、`commandcode` 与 `aliyun-bailian-token-plan` 的 provider smoke，不用于恢复已删除插件。
...
Generated-asset tests 再验证紧凑目录：当前 v41 索引包含 3 个域（`writing`、
