...
4. Size each node to fit its exact label plus padding, set graph-level ELK layoutOptions (algorithm, direction, edge routing), and regenerate from the IR. The engine applies tuned spacing, padding, and compaction defaults; only override spacing options for specific needs. Fix overlap, clipping, or crossings by changing ELK layout options or node sizes and regenerating, never by editing coordinates. Keep icon and label nodes separate; keep topology, text, and connectors in TikZ.
