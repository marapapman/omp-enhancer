...
- 本机 marketplace 安装的 0.1.96 最终复测通过：复合写作加测试加发布请求返回 `writing.pending/required/required/advisory`，嵌套 A/B 写作探针加安全审计返回 `security-review/true/advisory`；独立的代码 bug 审计与文档事实核查分别返回 `bug-audit` 和 `fact-check`。
- 复杂复合探针第一次虽然路由值正确，但模型额外添加了 Markdown 包装。将输出约束改写为明确的“单行纯文本、无 Markdown、严格模板”后，同一已安装版本精确输出目标行。该问题通过提示词收敛，没有增加格式门禁、拒绝路径或重试控制器。
- 0.1.96 的隔离真实编辑复测只把 `reliable advisory workflow` 改为 `robust advisory workflow`，返回 `EDIT_OK`，目录中只有目标文件；执行过程中没有写入拒绝、门禁重试或自动续跑，临时验证目录随后已清理。
