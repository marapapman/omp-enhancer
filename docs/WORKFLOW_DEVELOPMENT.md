...
- 如果只是语言、框架、数据库、构建或领域知识，复用 `plan`、`implementation-task`、`reviewer` 等 canonical role，并新增或复用 Skill。
...
- Main 是 workflow 编排所有者。顶层 Agent 禁止 `spawns: "*"`，spawn 目标必须是唯一打包 ID；当前 canonical Agent 默认不做隐藏的二次编排。
...
真实 OMP E2E 还要实际启动 OMP，覆盖工作树插件加载、native todo/task batch、精确 context、assignment 元数据和子 Agent 最终完成状态：
...
Matrix 使用 `noExtensions: true` 和显式 `pluginDirs` 时可直接验证当前工作树，避免本地 marketplace cache 让旧安装态产生假绿。`minNativeTaskAssignmentAttempts` 与 `maxNativeTaskAssignmentAttempts` 限制 assignment 尝试数；`requireNativeTaskCompletion` 另外要求对应异步 job 最终为 `completed`。场景应检查 required/forbidden Agent、workflow 和 Skill 元数据；涉及外部写入的 workflow 必须在临时目录中验证，或只做明确标为 preview 的 dry-run，不能为了 E2E 自动发布。
