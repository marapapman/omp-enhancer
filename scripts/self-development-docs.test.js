...
test('current repository documentation matches the v39 runtime and evidence contracts', async () => {
  ...
  assert.match(agents, /Workflow catalog \(v39\)/u);
  assert.match(workflows, /catalog v39 只有 3 个 ID/isu);
  assert.doesNotMatch(
...
test('omp-config README matches the current catalog and staged Beamer workflow', async () => {
  ...
  assert.match(readme, /generated from `scripts\/workflow-definitions\.js`.+catalog v39/isu);
