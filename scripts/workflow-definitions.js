...
const writingWorkflows = [
  {
    ...
    suggestedFlow: [
      ...
      'For Word output, apply the approved content through officecli (docx skill) and verify with officecli validate and a rendered view.',
...
const visualWorkflows = [
  {
    ...
    scopeNotes: [
      ...
      'Style follows paper/report norms over marketing decks: monochrome base (black/white/gray; gray steps belong to the base family, not an accent color) — white or transparent background, gray-scale box lines and text — with at most ONE additional accent color used only where it carries real semantic weight (a single highlight or a single contrast pair); hard bans on the AI rainbow: per-element or per-category multi-color assignment (a different color per node, layer, or series), the colorblind-safe/dark/heatmap multi-color style presets, and gradients; line style (solid/dashed), weight, and labels carry distinctions instead of hue. When drawing, set explicit node styles (fillColor=none or white, black/gray strokes, gradientColor=none) to override the skill\'s built-in palette, and run autolayout.py with --mono (its default is per-group color rotation). No drop shadows, decorative banner titles, or a pastel per layer. Layering and symmetry come from the real structure, not a fixed three-column or three-tier template. User-provided templates or house styles win over these defaults; vector primitives are the default, and raster assets stay opt-in per the asset rules below.',
