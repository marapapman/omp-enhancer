...
export function normalizeRelativePath(value, label = 'path') {
  if (typeof value !== 'string' || value.trim() === '') {
    throw new MermaidRuntimeError('INVALID_PARAMETER', `${label} must be a non-empty project-relative path.`);
  }
  if (value.includes('\0')) {
    throw new MermaidRuntimeError('INVALID_PARAMETER', `${label} contains a NUL byte.`);
  }
  ...
  if (
    ...
    throw new MermaidRuntimeError('PATH_OUTSIDE_PROJECT', `${label} must stay inside the project root.`, {
      path: value,
    });
  }
  ...
  if (clean === '') {
    throw new MermaidRuntimeError('INVALID_PARAMETER', `${label} must identify a file or directory.`);
  }
  return clean;
}
...
export async function resolveProjectRoot(projectRoot) {
  if (typeof projectRoot !== 'string' || projectRoot.trim() === '') {
    throw new MermaidRuntimeError('INVALID_PARAMETER', 'A project root is required.');
  }
  let root;
  try {
    ...
    throw new MermaidRuntimeError('INVALID_PROJECT_ROOT', `Project root is not an existing directory: ${projectRoot}`, {
...
export async function resolveExistingProjectFile(projectRoot, relativePath, label = 'path') {
  ...
  if (!isInside(root, lexicalPath)) {
    throw new MermaidRuntimeError('PATH_OUTSIDE_PROJECT', `${label} escapes the project root.`, { path: relativePath });
  }
  ...
  try {
    ...
    if (missing(error)) {
      throw new MermaidRuntimeError('FILE_NOT_FOUND', `${label} does not exist: ${normalized}`, { path: normalized });
    }
    throw error;
  }
  if (!isInside(root, resolved)) {
    throw new MermaidRuntimeError('SYMLINK_ESCAPE', `${label} resolves outside the project root.`, {
      path: normalized,
    });
  }
  const metadata = await stat(resolved);
  if (!metadata.isFile()) {
    throw new MermaidRuntimeError('INVALID_FILE', `${label} must identify a regular file.`, { path: normalized });
  }
...
export async function ensureProjectDirectory(projectRoot, relativePath, label = 'directory') {
  ...
  for (const segment of segments) {
    ...
    if (!isInside(root, current)) {
      throw new MermaidRuntimeError('PATH_OUTSIDE_PROJECT', `${label} escapes the project root.`, {
        path: normalized,
      });
    }
    try {
      ...
      if (metadata.isSymbolicLink()) {
        ...
        throw new MermaidRuntimeError('INVALID_DIRECTORY', `${label} crosses a non-directory path.`, {
          path: normalized,
        });
      }
    } catch (error) {
...
export async function assertWritableProjectFile(projectRoot, relativePath, label = 'path') {
  ...
  if (!isInside(directoryResult.root, candidate)) {
    throw new MermaidRuntimeError('PATH_OUTSIDE_PROJECT', `${label} escapes the project root.`, { path: normalized });
  }
  try {
    const metadata = await lstat(candidate);
    if (metadata.isSymbolicLink()) {
      throw new MermaidRuntimeError('SYMLINK_ESCAPE', `${label} must not be a symbolic link.`, { path: normalized });
    }
    if (!metadata.isFile()) {
      throw new MermaidRuntimeError('INVALID_FILE', `${label} must identify a regular file.`, { path: normalized });
    }
