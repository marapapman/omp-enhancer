...
function validateMermaidParams(input) {
  ...
  if (hasSource === hasSourcePath) {
    throw new MermaidRuntimeError('INVALID_PARAMETER', 'Provide exactly one of source or sourcePath.');
  }
  const theme = input.theme ?? 'default';
  if (!THEMES.includes(theme)) {
    throw new MermaidRuntimeError('INVALID_PARAMETER', 'theme must be one of: default, forest, dark, neutral.');
  }
  if (input.width !== undefined && (!Number.isInteger(input.width) || input.width <= 0)) {
    throw new MermaidRuntimeError('INVALID_PARAMETER', 'width must be a positive integer.');
  }
  return {
    hasSource,
    theme,
...
async function readBoundedSource(path) {
  ...
  if (metadata.size > MAX_SOURCE_BYTES) {
    throw new MermaidRuntimeError('SOURCE_TOO_LARGE', 'A Mermaid source exceeds the 2 MiB safety limit.', {
      path,
      bytes: metadata.size,
    });
  }
...
async function runCheckedCommand(commandRunner, args, options) {
  ...
  if (!evidence || evidence.exitCode !== 0) {
    ...
    const stderr = typeof evidence?.stderr === 'string'
      ...
      : '';
    throw new MermaidRuntimeError('COMMAND_FAILED', 'mermaid-cli did not report a successful exit.', {
...
export async function renderMermaid(input = {}, options = {}) {
  ...
  if (params.hasSource) {
    if (input.source.includes('\0')) {
      throw new MermaidRuntimeError('INVALID_PARAMETER', 'source must not contain NUL bytes.');
    }
    if (Buffer.byteLength(input.source, 'utf8') > MAX_SOURCE_BYTES) {
      throw new MermaidRuntimeError('SOURCE_TOO_LARGE', 'Mermaid source exceeds the 2 MiB safety limit.');
    }
    source = input.source;
    sourceBase = 'diagram';
    ...
    if (extension !== '.mmd' && extension !== '.md') {
      throw new MermaidRuntimeError('INVALID_PARAMETER', 'sourcePath must identify a .mmd or .md file.');
    }
    if (basename(resolved.path).startsWith('-')) {
      throw new MermaidRuntimeError('INVALID_PARAMETER', 'sourcePath basename must not begin with a dash.');
    }
  ...
  if (!cliPath) {
    throw new MermaidRuntimeError('MERMAID_NOT_INSTALLED', MERMAID_INSTALL_GUIDANCE, {
      package: MERMAID_CLI_PACKAGE,
    });
  }
  const chromePath = await (options.probeChrome ?? defaultProbeChrome)();
  if (!chromePath) {
    throw new MermaidRuntimeError('CHROME_NOT_FOUND', CHROME_NOT_FOUND_GUIDANCE);
  }
