...
9. Have `task` validate and integrate the complete designer revision against the committed outline, output language, source facts, semantic anchors, and LaTeX structure without taking layout or source-revision ownership. If validation finds an unintended content, frame-order, or scope change, return it to `designer` for correction instead of modifying the visual revision in task.
10. Have `task` recompile and render that exact designer revision. Task binds one revision identifier, the PDF, the render directory, fresh renders of every page, and an overview or contact sheet. Do not give `visioner` pre-designer or mixed-revision evidence.
...
12. Treat `CHANGES_REQUIRED` as advisory. For a supported finding, `designer` applies the bounded source revision, `task` rerenders it, and `visioner` reviews only fresh rerendered evidence, at most once for that changed revision. Do not redispatch automatically or review an unchanged artifact. Report material findings that remain.
...
6. Have `task` validate and integrate the complete designer revision against the requested semantic diff, LaTeX anchors, and authorized scope without taking layout or source-revision ownership. If validation finds unintended wording, math, citations, frame order, or unrelated changes, return it to `designer` for correction instead of modifying the visual revision in task.
7. Have `task` recompile and render that exact designer revision. Task binds one revision identifier, the PDF, the render directory, fresh high-resolution renders of every affected page, and a current full-deck overview or contact sheet. If a shared template, style, or macro changed, render and review every page.
...
9. Treat `CHANGES_REQUIRED` as advisory. For a supported finding, `designer` applies the bounded source revision, `task` rerenders it, and `visioner` reviews only fresh rerendered evidence, at most once for that changed revision. Do not redispatch automatically or review an unchanged artifact. Report unresolved blocker or major findings honestly.
