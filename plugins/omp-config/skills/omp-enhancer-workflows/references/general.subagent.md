...
- Reference steps:
  ...
  4. [step-audit] For a mutation or creation delivery, reviewer independently audits the bounded diff and evidence without project reads or commands; for pure read-only analysis, Main records fallback=parent-owned action with the reason instead of a second auditor
  5. [step-verify] Main owns final verification against the acceptance criteria plus all permission and external-effect decisions.
  6. [step-report] Report the integrated result, acceptance evidence, and material limitations.
