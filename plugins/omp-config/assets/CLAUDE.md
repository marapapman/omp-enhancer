...
For substantive code mutation using the loaded code method, parent-owned pre-dispatch prerequisites are bounded evidence briefs delegated to scout (local) and librarian (external), and a detailed dependency-wave plan of non-overlapping vertical slices delegated to native `plan`, which drafts the parallel plan; native `reviewer` audits plan output before production mutation. Each native `task` slice owns test mutation, valid RED, minimum production, the same-command GREEN, refactor, and returned evidence. Main integrates deliveries, verifies the current tree, and writes `MAIN REVIEW` before native `reviewer` receives only the Main-reviewed bounded diff and evidence. The reviewed diff covers the complete change, including code Main wrote itself; plan audit and code review fall back only when the named Agent is unavailable, recorded on the affected row. Main validates findings; a supported finding returns to `task` as a bounded repair, followed by refreshed evidence, another Main review, and at most one fresh affected reviewer pass. This code lifecycle does not apply to another domain unless its loaded resources name it.
