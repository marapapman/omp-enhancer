 ...
 * Each plugin maintains its own copy to respect the no-cross-plugin-dependency invariant.
 */
...
export function requireString(toolName, field, value) {
  if (value === null || value === undefined || value === '') {
    return { ok: false, error: actionableParamError({ toolName, field, badValue: value, expected: 'a non-empty string', example: `"${field}": "some-value"` }) };
  }
  if (typeof value !== 'string') {
    return { ok: false, error: actionableParamError({ toolName, field, badValue: value, expected: 'a string', example: `"${field}": "some-value"` }) };
  }
...
export function parseJsonParam(toolName, field, value, opts = {}) {
  ...
  if (typeof value !== 'string') {
    return { ok: false, error: actionableParamError({ toolName, field, badValue: value, expected: 'a JSON string (use JSON.stringify() to convert your object)', example: `"${field}": ${JSON.stringify(JSON.stringify({ id: 'root' }))}` }) };
  }
  if (value.trim() === '' && !allowEmptyString) {
    return { ok: false, error: actionableParamError({ toolName, field, badValue: value, expected: 'a non-empty JSON string' }) };
  }
