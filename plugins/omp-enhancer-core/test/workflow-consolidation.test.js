...
test('compact workflow Skill teaches primary-plus-add-on composition with exact resource URIs', () => {
  ...
  assert.match(index, /PLAN reads NOW\/waits[\s\S]*THEN is one final unsplit resource-only batch\/wait[\s\S]*NOW=none[\s\S]*Give each evidence checkpoint an Action/iu);
