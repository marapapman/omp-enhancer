...
test('a verified exact workflow index read presents PRE_PLAN until assistant progress', () => {
  ...
  assert.match(first.content, /D\/C[\s\S]*optional candidates/u);
  assert.match(first.content, /smallest matching exact URI set/u);
  assert.doesNotMatch(first.content, /writing\.en|writing-review|Agent=/u);
...
test('CRLF PLAN freezes declared reads and waits for all resources and final THEN', () => {
  ...
  assert.match(cue.content, /byte 0[\s\S]*WORKFLOW READY/u);
  assert.match(cue.content, /TODO only[\s\S]*wait/u);
  assert.doesNotMatch(cue.content, /writing\.en|writing-review|ZhWriter/u);
});
...
test('READY must be at byte zero and only its successful TODO queues PRE_DISPATCH', () => {
  ...
  const retry = presentWorkflowProtocolCoachCue(state);
  assert.deepEqual(retry, first);
  assert.match(first.content, /committed `tasks\[\]` batch form[\s\S]*nonempty top-level `context`/u);
  assert.match(first.content, /each item uses only exposed fields[\s\S]*Agent into `agent`/u);
  const assignmentPrefix = '[workflow=<copy-workflow> step=<copy-step> todo=<copy-checkpoint-verbatim> skills=<copy-skills>]';
  assert.equal(first.content.split(assignmentPrefix).length - 1, 1, 'the complete assignment prefix appears exactly once');
  assert.match(first.content, /committed Delegate row[\s\S]*mechanically[\s\S]*exact key order/u);
  assert.match(first.content, /complete terminal child delivery[\s\S]*directly usable artifact\/evidence/u);
  assert.match(first.content, /status-only or artifact-reference-only/u);
  assert.match(first.content, /conditional repair checkpoint[\s\S]*no accepted finding[\s\S]*resolved\/completed[\s\S]*not drop\/abandon/u);
  assert.doesNotMatch(first.content, /Every native `task` call|task metadata/u);
  assert.match(first.content, /selects no Agent, fork, dispatch, retry, or completion/u);
  observeProtocolToolCall(state, { name: 'task' });
  assert.equal(state.pendingCue, null);
});
