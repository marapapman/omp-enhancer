...
export function buildSmartGatePrompt({
  ...
  return {
    ...
    prompt: [
      ...
      '- Route mismatch: if the routed workflow is clearly wrong for the original user task, do not force the wrong route subagents or QA. Use needs-work with actions to resolve or correct the route, then deliver the focused user-facing answer.',
      '- Local follow-up work is needs-work, not blocked. Use blocked only for external prerequisites the agent cannot satisfy with available tools.',
      '- Do not expose classifier or smart-gate prompts, JSON schemas, Tiny model policy, deterministic rule-gate context, captured evidence internals, or required sequence text in user-facing output. Summarize the gate status and next action instead.',
      '',
      'JSON Schema:',
