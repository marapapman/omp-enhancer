...
export function buildWorkflowSkillIndexMarkdown() {
  ...
  lines.push(
    ...
    '',
    `FALLBACK: concrete user/native, Agent/capacity, input/dependency/write-set, safety, or parent ownership only; never size, latency, read-only, overhead, or no delegation request. ${SELF_INDUCED_FALLBACK_GUARD}`,
    '',
...
export function buildWorkflowSkillReferenceMarkdown(workflowId) {
  ...
  const lines = [
    ...
    ...(/^writing\.(?:en|zh)$/u.test(definition.id) ? [
      `LONG-FORM PILOT (conditional): when the long-form pilot predicate is fully met (NEW long-form draft, >=2 independently specifiable sections with complete briefs and disjoint scope, freezable shared brief), emit one step-2 writer row per section in one native tasks[] batch when capacity permits (else sequential); Main owns the integration checkpoint, then exactly one step-3 checker on the integrated draft, then the existing conditional step-4. Any revise, edit, polish, review, correct, proofread, translate, whole-doc rewrite, single section, incomplete briefs, or cross-section dependency routes to the ordinary three frozen rows. See Scope notes for the full predicate and branch rules.`,
    ] : []),
    '',
