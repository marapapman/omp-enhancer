...
import type { ExtensionAPI, ExtensionToolContext } from './ompApi.js'
...
export function registerTestingEnhancer(pi: ExtensionAPI): void {
  ...
  const prepareStateForContext = (ctx: ExtensionToolContext): void => {
    ...
    if (!taskContextIdentity || currentState.taskContextIdentity === taskContextIdentity) return
  ...
  const recordBrowserCheckOutput = async (output: BrowserEvidence, ctx: ExtensionToolContext): Promise<void> => {
    prepareStateForContext(ctx)
    const taskContextIdentity = readCoreTaskContextIdentityFromEntries(branchEntries(ctx))
      ?? currentState.taskContextIdentity
      ?? `testing:${output.runId ?? 'browser-unscoped'}`
    currentState = recordObservedBrowserEvidence(
      scopeTestingReviewToTaskContext(currentState, taskContextIdentity),
      {
        schemaVersion: 2,
        taskContextIdentity,
  ...
  const clearObservedBrowserEvidence = async (_event: unknown, ctx: ExtensionToolContext): Promise<void> => {
    prepareStateForContext(ctx)
    if (!currentState.lastObservedBrowserEvidence) return
    currentState = invalidateObservedBrowserEvidence(currentState)
    await persistTestingReview(ctx)
  }
