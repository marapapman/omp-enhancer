...
export function trustedHostCommand(event: Record<string, unknown>): string | undefined {
...
export function isTrustedExplicitTestAttempt(event: Record<string, unknown>): boolean {
  const command = trustedHostCommand(event)
  return Boolean(command && isExplicitStandaloneTestCommand(command))
}

export function isExplicitStandaloneTestCommand(command: string): boolean {
...
export function hasUnsafeShellControlSyntax(command: string): boolean {
...
export function isNonExecutingTestProbe(command: string): boolean {
  return /(?:^|\s)(?:--help|-h|--listtests|--list-tests|--collect-only|--passwithnotests)(?:\s|$)/i.test(command)
}

export function isExplicitPositiveTestOutput(value: string, command: string): boolean {
...
export function hostToolResultText(event: Record<string, unknown>): string {
  return [
    event.output,
    event.stdout,
    event.content,
...
export function isDefiniteWorkspaceMutationHostEvent(event: Record<string, unknown>): boolean {
  const name = String(event.name ?? event.toolName ?? '').toLowerCase()
