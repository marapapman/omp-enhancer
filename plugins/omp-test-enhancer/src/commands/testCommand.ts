export type TestCommandMode =
  | { kind: 'run'; files: string[] }
  | { kind: 'check' }
  | { kind: 'report' }
  | { kind: 'init' }
  | { kind: 'help' }
...
export function parseTestCommandMode(args: string): TestCommandMode {
  const parts = args
    .trim()
    .split(/\s+/)
    .map(item => item.trim())
  ...
  if (!first) return { kind: 'run', files: [] }
  ...
  if (first === 'report') return { kind: 'report' }
  if (first === 'init') return { kind: 'init' }

  return { kind: 'run', files: parts }
}

export function buildTestHelpText(): string {
  return [
    ...
    '',
    '子代理顺序：',
    '1. test-planner 只读分析目标、公开行为和证据需求，输出 TEST_PLAN。',
    '2. test-executor 按确认范围修改测试和 fixtures，通过宿主运行真实命令并记录当前证据。',
    '3. test-reviewer 独立只读审查计划、测试 diff、公开行为覆盖和当前证据，不修改文件，也不重跑测试。',
    '4. 父级工作流汇总 findings。插件不自动调度修复或继续会话。',
    '',
    '工作规则：',
    '1. 默认只修改测试文件。',
