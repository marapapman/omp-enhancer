 ...
 */
export function readCoreRouteIdentityFromEntries(entries: SessionEntry[]): string | undefined {
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    const entry = entries[index]
    if (entry?.type !== 'custom' || entry.customType !== CORE_STATE_ENTRY) continue
    if (!isRecord(entry.data) || entry.data.schemaVersion !== 2) continue
    if (!isRecord(entry.data.lastRoute) || typeof entry.data.lastRoute.intent !== 'string') continue
