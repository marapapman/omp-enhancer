...
describe('buildTestHelpText', () => {
  it('documents only /test commands without dashed variants', () => {
    ...
    expect(help).toContain('test-planner')
    expect(help).toContain('test-executor')
    expect(help).toContain('test-reviewer')
    expect(help.indexOf('test-planner')).toBeLessThan(help.indexOf('test-executor'))
    expect(help.indexOf('test-executor')).toBeLessThan(help.indexOf('test-reviewer'))
    expect(help).toContain('独立只读审查')
