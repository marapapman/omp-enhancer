...
describe('advisory route scope e2e', () => {
  it('scopes review evidence to the current core v2 user turn', async () => {
    ...
    expect(latestEntry(pi, TESTING_EVIDENCE_ENTRY)).toMatchObject({
      routeIdentity: 'route:101',
      reviewStatus: 'ready',
      advisory: true
    })

    const evidenceCount = entriesOfType(pi, TESTING_EVIDENCE_ENTRY).length
    pi.entries.push(coreRouteEntry(202, 'bug-audit'))
  ...
  it('uses a local diagnostic identity when no core route is present', async () => {
    const pi = new FakePi()
    registerTestingEnhancer(pi)
    const ctx = context(pi)
    ...
    expect(latestEntry(pi, TESTING_EVIDENCE_ENTRY)).toMatchObject({
      routeIdentity: expect.stringMatching(/^testing:test-/),
      reviewStatus: 'collecting',
      advisory: true
    })
  })
...
function coreRouteEntry(routeStartedAt: number, intent: string) {
  return {
