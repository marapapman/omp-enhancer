...
export async function executeBrowserCheck(input, ctx) {
    const parsed = parseBrowserCheckParams(input);
    if (!parsed.ok)
        return invalidBrowserCheckEvidence(input, parsed.error);
    ...
    try {
        ...
        page.on('console', message => {
            ...
            if (type === 'error')
                consoleErrors.push({ message: message.text(), type });
            if (type === 'warning')
                consoleWarnings.push({ message: message.text(), type });
        });
        ...
        if (server) {
            if (!serverClosed && server.exitCode === null)
                server.kill();
            await serverClosePromise;
        }
    }
}
export function isAllowedBrowserServerCommand(command) {
    ...
    if (values[0] === 'run')
        return /^(?:start|dev|serve|preview)$/.test(values[1] ?? '');
    return /^(?:start|dev|serve|preview)$/.test(values[0] ?? '');
}
export async function resolveSafeBrowserArtifactDir(cwd, requested, runId) {
    try {
        ...
        if (!pathIsWithin(trustedRoot, candidate))
            return null;
        let existingAncestor = candidate;
        while (pathIsWithin(trustedRoot, existingAncestor)) {
            try {
                const resolvedAncestor = await realpath(existingAncestor);
                if (!pathIsWithin(trustedRoot, resolvedAncestor))
                    return null;
                break;
            }
            catch {
                if (existingAncestor === trustedRoot)
                    return null;
...
function serverCommandFailureFinding(command, error) {
    return {
        gate: 'browser-interaction',
        passed: false,
        severity: 'critical',
...
export function normalizeBrowserFindings(input) {
    const findings = [];
...
export async function executeBrowserScenarios(page, params, artifactDir) {
    const findings = [];
    const artifacts = {};
    const executionCounts = { scenarioCount: 0, stepCount: 0, captureCount: 0, visualAssertionCount: 0 };
    for (const [scenarioIndex, scenario] of params.scenarios.entries()) {
        ...
        if (!scenarioFailed)
            executionCounts.scenarioCount += 1;
    }
    return { executionCounts, findings, artifacts };
}
async function executeStep(page, step, baseUrl, artifactDir, scenarioName, artifacts, executionCounts) {
    if (step.action === 'goto') {
    ...
    if (step.action === 'screenshot') {
        ...
        if (!artifacts.actualImagePath)
            artifacts.actualImagePath = path;
...
async function executeVisualCheck(page, visualCheck, artifactDir, findings, artifacts, executionCounts) {
    ...
    if (visualCheck.kind === 'locator' && locator)
        await locator.screenshot({ path: actualImagePath });
    else
        await page.screenshot({ path: actualImagePath, fullPage: true });
    if (!artifacts.actualImagePath)
        artifacts.actualImagePath = actualImagePath;
    executionCounts.captureCount += 1;
    if (!visualCheck.expectedPath)
        return;
    ...
    if (typeof visualCheck.maxDiffPixels === 'number')
        compareOptions.maxDiffPixels = visualCheck.maxDiffPixels;
    if (typeof visualCheck.maxDiffPixelRatio === 'number')
        compareOptions.maxDiffPixelRatio = visualCheck.maxDiffPixelRatio;
...
function resolveLocator(page, locator) {
    ...
    if (locator.kind === 'text')
        return page.getByText(locator.value ?? '', locatorExactOption(locator));
    if (locator.kind === 'label')
        return page.getByLabel(locator.value ?? '', locatorExactOption(locator));
    if (locator.kind === 'placeholder')
        return page.getByPlaceholder(locator.value ?? '', locatorExactOption(locator));
    if (locator.kind === 'altText')
        return page.getByAltText(locator.value ?? '', locatorExactOption(locator));
    if (locator.kind === 'title')
        return page.getByTitle(locator.value ?? '', locatorExactOption(locator));
...
function locatorExactOption(locator) {
    const option = {};
    if (locator.exact !== undefined)
        option.exact = locator.exact;
    return option;
}
...
function buildEvidence(params, runId, headless, viewport, findings, artifacts, executionCounts) {
    const hasCriticalFinding = findings.some(finding => !finding.passed && finding.severity === 'critical');
