...
function trustedHostCommand(event) {
...
function isTrustedExplicitTestAttempt(event) {
    const command = trustedHostCommand(event);
...
function isExplicitStandaloneTestCommand(command) {
...
function hostToolResultText(event) {
    return [
        event.output,
        event.stdout,
        event.content,
...
function isDefiniteWorkspaceMutationHostEvent(event) {
    const name = String(event.name ?? event.toolName ?? '').toLowerCase();
