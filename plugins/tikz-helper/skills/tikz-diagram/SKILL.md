...
3. Author the semantic graph as an ELK graph IR: nodes sized for their exact labels plus padding, single source/target edges, and graph-level `layoutOptions` carrying `elk.algorithm`, `elk.direction`, spacing, and `elk.edgeRouting`. Input nodes must omit x and y and input edges must omit sections and bendPoints; the layout engine computes them. Model groups as parent nodes with `children`; cross-parent edges need layered with `elk.hierarchyHandling: INCLUDE_CHILDREN`.
