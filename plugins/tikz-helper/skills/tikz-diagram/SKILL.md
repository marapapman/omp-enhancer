...
4. `task` calls `tikz_generate_diagram` with the designer-produced ELK graph IR to compute the layout with ELK and emit the compilable TikZ source. The tool returns the `.tex` source and positioned graph metadata; it is the sole geometry source, not a hand-draft surface.
5. `task` writes the returned `.tex` to the user-authorized project path. The engine applies tuned spacing, padding, and compaction defaults; pass the chosen preset, density, and any explicit targetWidthPt to the tool. If overlap or clipping persists, adjust specific ELK layout options or node sizes and regenerate, never by editing coordinates. The author never authors, infers, or hand-edits TikZ coordinates. Treat imagegen, exposed as `generate_image`, as an optional icon source only under the method below; Main authorizes that optional external effect during initial setup and `task` invokes it.
...
4. **Designer ELK IR checkpoint** — Only after asset review, `designer` authors the semantic graph as an ELK graph IR with layout options, node sizing, and the `visioner`-approved asset manifest integrated. The designer returns the graph IR JSON; it does not invoke `tikz_generate_diagram` or hand-edit coordinates.
5. **Task generation checkpoint** — `task` receives the approved ELK graph IR, calls `tikz_generate_diagram` to compute layout and emit TikZ source, writes the returned `.tex` to the user-authorized project path, and verifies the semantic-graph round-trip. The author never hand-edits the coordinates the tool returns.
6. **Task render checkpoint** — After `task` returns the complete manifest-bound source revision, `task` invokes `tikz_render`: it validates project-relative paths, copies the dependency graph to a temporary workspace, and binds fresh exact revision PDF, SVG, full-size PNG, and 60% PNG plus structured command evidence.
7. **Visioner figure-review checkpoint** — Only after the designer delivery is integrated and task renders exist, `visioner` independently and read-only checks layout and legibility on the fresh current-revision full-size and 60% renders against the semantic spec and asset manifest.
8. **Designer-visioner loop** — `designer` applies supported findings by revising the ELK graph IR, `task` regenerates via `tikz_generate_diagram` and rerenders via `tikz_render`, and `visioner` reviews only fresh rerenders at most once per changed revision. Main authorizes external-effect decisions during initial setup and accepts the final delivery; it does not render, modify, reconcile, or mediate the visual loop.
