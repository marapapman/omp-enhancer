...
Fix overlap, clipping, or crossings by changing ELK layout options or node sizes and regenerating, never by editing coordinates. A geometry finding returns the author to the ELK IR sizes and layout options; `task` regenerates from the revised IR and rerenders, and the review judges only the fresh regenerated evidence.
When a preset was used, compare the rendered size against `metadata.sizing` (targetWidthPt, scale, effectiveFontPt). Density relayouts and sub-6pt effective-font warnings are advisory findings — report them to Main, never fix by editing coordinates.
